...
type Bus
   ...
   ancestor::Vector{Int}
   generator::Generator
   function Bus(index, d_P, d_Q, v_max, v_min, p_PV)
      b = new()
      b.index = index
      b.is_root = false
      b.d_P = d_P
      ...
      b.children = Int[]
      b.ancestor = Int[]
      cosphi = 0.9#d_P/(sqrt(d_P^2 + d_Q^2))
...
for n in 1:nrow(nodes_raw)
    ...
    p_PV = nodes_raw[n, :PV]
    newb = Bus(index, d_P, d_Q, v_max, v_min, p_PV)
